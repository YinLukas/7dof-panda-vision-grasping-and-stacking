...
class IK:
    ...
    def distance_and_angle(G, H):
        ...
        # numerical safety to avoid domain errors in acos
    ...
    def is_valid_solution(self, q, target):
        ...
        success - a Boolean which is True if and only if the candidate solution
                  produces an end effector pose which is within the given linear
                  and angular tolerances of the target pose, and also respects
                  the joint limits.
        message - a string describing whether success or the failure reason
        """
        ...
        if distance >= self.linear_tol:
            return False, f"Failed: position error {distance:.4f} exceeds {self.linear_tol}"

        # Check orientation tolerance
        if angle > self.angular_tol:
            return False, f"Failed: orientation error {angle:.4f} exceeds {self.angular_tol}"

        # Check joint limits
        for i in range(len(q)):
            if q[i] < self.lower[i] or q[i] > self.upper[i]:
                return False, f"Failed: joint {i} out of limits ({q[i]:.3f})"

        # If all checks pass
        return True, "Success: within tolerance and joint limits"
        ## END STUDENT CODE
    ...
    def end_effector_task(q, target, method):
        ...
        v = np.reshape(displacement, (3, 1))
        w = np.reshape(axis, (3, 1))
        vw_ee = np.vstack((v, w))
        J = calcJacobian(q)
        if method == 'J_pseudo':
            dq = IK_velocity(q, v, w)
        elif method == 'J_trans':
            alpha = 1
            dq = alpha * J.T @ vw_ee
    ...
    def inverse(self, target, seed, method, alpha):
        ...
        while True:
            ...
            dq = dq_ik + (np.eye(7) - J_pseudo @ J) @ dq_center # dq velocity
            # Check termination conditions
            if np.linalg.norm(alpha * dq) < self.min_step_size or step >= self.max_steps:
                break
            # update q
            q = q + alpha * dq.flatten()
            step += 1
        ## END STUDENT CODE
